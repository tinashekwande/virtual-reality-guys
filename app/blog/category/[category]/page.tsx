...
import { getPublishedPosts, getSidebarData, getCategoryBySlug } from "@/lib/blog"
import Link from "next/link"
import { ArrowLeft, ArrowRight, FolderOpen, SearchX } from "lucide-react"
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${categoryName} Articles | Virtual Reality Guys`,
    description: `Read our articles and guides on ${categoryName} in Cape Town. Explore how mobile VR brings immersive gaming to events.`,
    alternates: {
      ...
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-24 border-b border-border overflow-hidden bg-secondary/10">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10 space-y-4">
          <span className="inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-widest text-primary px-3 py-1 rounded-full border border-primary/20 bg-primary/5">
            <FolderOpen className="h-3.5 w-3.5" /> Category Archive
          </span>
          ...
          <p className="max-w-2xl mx-auto text-muted-foreground text-sm sm:text-base leading-relaxed">
            Browse our list of resources, guides, and tips related to <strong className="text-foreground">{categoryName}</strong>.
          </p>
          <div className="pt-2">
          ...
          <div className="lg:col-span-8 space-y-10">
            {posts.length === 0 ? (
              <div className="text-center py-20 bg-card border border-border border-dashed rounded-xl">
                ...
                <h3 className="font-tech text-lg font-bold text-foreground mb-2">No Articles in Category</h3>
                <p className="text-muted-foreground text-sm max-w-md mx-auto mb-6">
                  We don't have any published blog posts in the {categoryName} category yet.
                </p>
